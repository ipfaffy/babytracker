...
def main():
    ...
    def sms():
        ...
        logged_message = Message.create(sender=user, receivedat=now, raw=str(inbound_message))
        ...
        if formula_regex.search(inbound_message) or breastmilk_regex.search(inbound_message):
            ...
            this_bottle = Bottle.create(message=logged_message, bottletype=bottle_type, quantityinoz=int(bottle_amount_in_msg))
            resp.message("Logged bottle of {}.".format(bottle_type_in_msg.lower()))

        if pee_regex.search(inbound_message) or poop_regex.search(inbound_message) or mixed_regex.search(inbound_message):
            # This is a diaper message    
            if (pee_regex.search(inbound_message) and poop_regex.search(inbound_message)) or mixed_regex.search(inbound_message):
                # Diaper is mixed
            ...
            this_diaper_contents = DiaperContent.create(diaper=this_diaper, contenttype=diaper_contents)
